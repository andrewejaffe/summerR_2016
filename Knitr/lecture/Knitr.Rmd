...
ptable = select(ptable, Beta, ci, p.value)
pander(ptable)
```


## Multiple Models 
OK, that's pretty good, but let's say we have all three models.  Another package called `stargazer` can put models together easily and print them out. But it doesn't work so well with *many* models together.  So let's use stargazer.  Again, you need to use `install.packages("stargazer")` if you don't have function.  
